	...
	*/
	...
	this.skillToAdd = function (inputArray) {
		for (let i = 0; i < inputArray.length; i += 1) {
			...
			if (inputArray[i][1] > 20) {
				print("[ÿc7AutoSkill.jsÿc0] Skill build index " + i + " has allocation count of " + inputArray[i][1] + " and it will be limited to 20");
				inputArray[i][1] = 20;
			}

			// Set satisfy condition as default if not specified
			if (inputArray[i][2] === undefined) inputArray[i][2] = true;
	...
	this.allocate = function () {
		...
		if (!getUIFlag(0x17)) {
			...
			if (addTo) {
				print("[ÿc7AutoSkill.jsÿc0] Using skill point in Skill ID: " + addTo);
				delay(100);
	...
	this.init = function (skillBuildOrder, save = 0) {
		...
		if (!this.skillBuildOrder || !this.skillBuildOrder.length) {
			print("[ÿc7AutoSkill.jsÿc0] No build array specified");
		...
		print("[ÿc7AutoSkill.jsÿc0] Finished allocating skill points");
