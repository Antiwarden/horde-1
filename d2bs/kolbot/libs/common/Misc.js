				...
				for (j = 0; j < bodyLoc.length; j += 1) {
					...
					if ([3, 7].indexOf(items[0].location) > -1 && tier > equippedItem.tier && equippedItem.classid !== 174) { // khalim's will adjustment
						...
						print(
							...
							" ÿc0(tier " + tier + ") | Remove " + Pickit.itemColor(equippedItem) + equippedItem.name.trim() + " ÿc0(tier " + equippedItem.tier + ")");
				...
				if (me.gold > 1000) {
					for (i = 0; i < 4; i += 1) {
						if (Config.BeltColumn[i] === "hp" && Config.MinColumn[i] > 0) {
							...
							if (needhp) {
								print("We need ÿc1Healing potions");
...
var Experience = {
	...
	log: function () {
		...
		string = "[Game: " + me.gamename + " " + getGameTime + "] [Level: " + me.getStat(12) + " (" + progress + "%)] [XP: " + gain + "] [Games ETA: " + runsToLevel + "]";
		...
		if (tier > 0 && bodyLoc) {
			for (j = 0; j < bodyLoc.length; j += 1) {
				...
				if ([3, 7].indexOf(items[0].location) > -1 && tier > equippedTier) {
					...
					print(
						"[ÿc7AutoEquipÿc0] Merc equip " + Pickit.itemColor(items[0]) + items[0].name.trim() +
						" ÿc0(tier: " + tier + ") | Remove " + Pickit.itemColor(equippedItem) + equippedItem.name.trim() + " ÿc0(tier: " + equippedTier + ")");
