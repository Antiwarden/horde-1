 ...
 *			character build template file located in libs/config/Builds/*. Any skill and stat points
 *			obtained as quest rewards are currently invisible to this script and must be spent manually.
...
var AutoBuild = new function AutoBuild () {
	...
	function applyConfigUpdates () {
		if (debug) {
			myPrint("Updating build configuration from level " + configUpdateLevel + " to " + me.charlvl)
		}
	...
	function getBuildType () {
		...
		if (!build) {
			myPrint("Config.AutoBuild.Template is either 'false', or invalid (" + build + ")");
	...
	function initialize () {
		...
		myPrint("Including build template " + template + " into " + currentScript);
		...
		// Resynchronize our Config object with all past changes made to it by AutoBuild system
		applyConfigUpdates();
	};
	...
	function myPrint () {
		...
		args.unshift("[ÿc4AutoBuild.jsÿc0]");
