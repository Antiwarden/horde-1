...
var Communication = {
	...
	OrgTorch: {
		...
		onReceiveCommand: function (nick, message) {
			...
			if (args[1] === "ask") {
				let isReady = Role.orgTorchCheck();
		...
		if (id == 55) {
			...
			// Synchro command
				...
				case "tp plz":
					...
					break;
				// We're comparing if that case's value is equal to our switch term.
				// Thus, if the msg contains "Have: ", compare msg to msg (do case),
				// otherwise compare msg to null (do-not case).
				case (msg.match("Have: ") ? msg : null):
					// Change msg from a string to an array of 0's and 1's representing waypoint possession.
					msg = msg.split("Have: ")[1].split("");
					msg.unshift(nick); // Add the waypoint owner's name to the front of the array.

					// Record the Team Member's list of Waypoint possession values.
					// Should we check to make sure this character's waypoint hasn't
					// been previously recorded?
					Waypoint.teamWaypoints.push(msg);
