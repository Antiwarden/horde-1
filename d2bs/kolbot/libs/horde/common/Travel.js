...
var Travel = {
	...
	// TODO: Add moving to exit without clearing after XX minutes.
	moveToExit: function (currentarea, targetarea, cleartype) {
	...
	// TODO: Add moving to exit without clearing after XX minutes.
	clearToExit: function (currentarea, targetarea, cleartype) {
	...
	toCanyon: function () {
		...
		let journal = getPresetUnit(74, 2, 357);
		...
		Pather.usePortal(46); // What area is this?

		Waypoint.clickWP();
	},

	travel: function (goal) { // 0-> 9, a custom waypoint getter function
		let homeTown, startAct;
		let nextAreaIndex, target, destination, unit;
		let clearPath, wpAreas = [], areaIDs = [];

		let walkTravel = false;
		...
		if (nextAreaIndex < areaIDs.length) { // If next area index is invalid, return true.
			...
			for (nextAreaIndex; nextAreaIndex < areaIDs.length; nextAreaIndex += 1) {
				...
				case 110: // Harrogath -> Bloody Foothills
					...
					break;
				default:
					// If traveling in a group (all other goals are tele only).
					if (goal <= 3 && me.diff === 0) {
						if (!me.inTown) {
				...
				// Check if the next area (which we are now in) has a waypoint.
				// If it does, grab it, go to town, wait for party, run chores,
				// take waypoint, and wait for the party.
				if (wpAreas.indexOf(areaIDs[nextAreaIndex]) > -1) {
		...
		try {
			while (!me.inTown) {
				...
				case 38: // Tristram
					...
					break;
				default:
					return false; // Unhandled location
				}
