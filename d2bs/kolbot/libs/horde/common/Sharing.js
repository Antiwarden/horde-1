...
var Sharing = {	
	...
	onReceiveCommand: function(nick, msg) {
		/*if (HordeSettings.Debug.Verbose.sharing) print("[ÿc5Sharing.jsÿc0] " + nick + " sent " + msg);*/
		...
		if (args.length >= 2) {
			if (args[1] === "gold") {
				...
				if (args.length >= 3) {
					...
					} else if (args[2] === "result") this.onReceiveGearResult(nick, args);
					else if (args[2] === "pick") {
						if (args.length === 3) this.receiveItem(nick);
						else if (args.length === 4 && args[3] == "done") this.receivedPickDone = true;
					}
				}
			} else if (args[1] === "rune") {
				if (args.length >= 4) {
					...
					if (args[2] === "need") this.onReceiveRuneNeedList(nick, JSON.parse(json));
					else if (args[2] === "offer") this.onReceiveRuneOfferList(nick, JSON.parse(json));
					else if (args[2] === "drop") {
						if (args[3] === "done") this.runeDropDone = true;
						else this.onReceiveRuneDropList(nick, JSON.parse(json));
					} else if (args[2] === "pick") {
	...
	onReceiveGoldCommand: function(profile, arg) {
		...
		if (!found) this.goldAnswers.push({profile: profile, cmd: arg});
		
		if (profile === me.profile) Communication.sendToList(HordeSystem.allTeamProfiles, "sharing gold " + arg);
	},
	
	getProfileCommand: function(profile) {
		for (var i = 0 ; i < this.goldAnswers.length ; i += 1) {
			if (this.goldAnswers[i].profile === profile) return this.goldAnswers[i].cmd;
		}
		return "";
	},
	...
	getProfilesForGoldCommand: function(cmd) {
		var profiles = [];
		for (var i = 0 ; i < this.goldAnswers.length ; i += 1) {
			if (this.goldAnswers[i].cmd === cmd) profiles.push(this.goldAnswers[i].profile);
		}
		return profiles;
	},	
	...
	receiveGold: function () {
		...
		if (goldPile) Pickit.pickItem(goldPile);
		
		delay(me.ping*2 + 50);

		if (Role.isMediumGold()) this.onReceiveGoldCommand(me.profile, "good");
	},
...
*/
	...
	shareGold: function() {
		if (HordeSystem.teamSize === 1) return;
		...
		if (need) {
			if (HordeSettings.Debug.Verbose.sharing) print("[ÿc5Sharing.jsÿc0] " + me.profile + " need gold");
			this.onReceiveGoldCommand(me.profile, "need");
		} else if (offer) this.onReceiveGoldCommand(me.profile, "offer");
		else this.onReceiveGoldCommand(me.profile, "good");
		...
		if (HordeSettings.Debug.Verbose.sharing) {
			for (var i = 0 ; i < this.goldAnswers.length ; i += 1) {
				print("[ÿc5Sharing.jsÿc0] " + this.goldAnswers[i].profile + " " + this.goldAnswers[i].cmd + " gold");
			}
		}
		...
		if (this.getProfilesForGoldCommand("need").length > 0 && this.getProfilesForGoldCommand("offer").length > 0) {
			...
			while(this.getProfilesForGoldCommand("need").length > 0 && this.getProfilesForGoldCommand("offer").length > 0) {
				if (this.getProfileCommand(me.profile) === "need") this.receiveGold();				
				else if (this.getProfileCommand(me.profile) === "offer") this.giveGold();
				else {
					delay(50);
					Party.wholeTeamInGame();
				}
	...
	giveTP: function (nick) {
		...
		if (!this.nickList) this.nickList = {};
	...
	onReceiveProfileGearCount: function(profile, gearCount) {
		...
		if (profile === me.profile) Communication.sendToList(HordeSystem.allTeamProfiles, "sharing gear have " + gearCount);
	},
	...
	onReceiveGearOffer: function(nick, tier, mercTier, lvlReq, json) {
		...
		if (Item.autoEquipCheckTier(sentItem, lvlReq, tier)) resultStr += "1 tier " + tier;
		else if (Item.autoEquipCheckMercTier(sentItem, lvlReq, mercTier)) resultStr += "1 merctier " + mercTier;
		else resultStr += "0";
	...
	hasReceivedAllGearAnswers: function(profiles) {
		for (var i = 0 ; i < profiles.length ; i += 1) {
			...
			if (profile !== me.profile) {
				if (!this.offeredGearAnswers[profile]) return false;
			}
		}
	...
	offerItem: function(item, targetProfiles) {
		if (HordeSettings.Debug.Verbose.sharing) {
			print("[ÿc5Sharing.jsÿc0] Offering " + Pickit.itemColor(item).trim() + item.name.trim() + " (gid : " + item.gid + ")");
		}
		...
		for (var i = 0 ; i < targetProfiles.length ; i+=1) {
			...
			if (profile !== me.profile) {
				...
				if (HordeSettings.Debug.Verbose.sharing) {
					print("[ÿc5Sharing.jsÿc0] Sending " + Pickit.itemColor(item).trim() + item.name.trim() + " to " + profile + " (tier: " + tier + " - merc tier: " + mercTier);
				}
		...
		if (HordeSettings.Debug.Verbose.sharing) {
			print("[ÿc5Sharing.jsÿc0] Waiting all gear answers for " + Pickit.itemColor(item).trim() + item.name.trim() + " gid : " + item.gid);
		}
		...
		if (HordeSettings.Debug.Verbose.sharing) {
			print("[ÿc5Sharing.jsÿc0] Received all gear answers for " + Pickit.itemColor(item).trim + item.name.trim() + " gid : " + item.gid);
		}
	},
		
	giveItem: function(profile, item) {
		if (HordeSettings.Debug.Verbose.sharing) HordeDebug.logScriptInfo("GearSharing", "dropping " + item.name + " for " + profile);
		...
		if (HordeSettings.Debug.Verbose.sharing) print("[ÿc5Sharing.jsÿc0] Wait receive pick done");
		...
		if (HordeSettings.Debug.Verbose.sharing) print("[ÿc5Sharing.jsÿc0] Received pick done");
	},
	
	receiveItem: function(profile) {
		if (HordeSettings.Debug.Verbose.sharing) HordeDebug.logScriptInfo("GearSharing", "picking item from " + profile);
		Town.goToTown(Party.lowestAct);
		Town.move("stash");
	...
	isInOfferedGearHistory: function(item) {
		for (var i = 0 ; i < this.offeredGearHistory.length ; i += 1) {
			if (item.gid === this.offeredGearHistory[i].gid && item.classid === this.offeredGearHistory[i].classid) return true;
		}
		return false;
	},
	...
	shareGear: function(fieldSharing) {
		if (HordeSystem.teamSize === 1 || !this.isGearSharingEnabled()) return;
		
		if (fieldSharing === undefined) fieldSharing = false;
		...
		if (HordeSettings.Debug.Verbose.sharing) print("[ÿc5Sharing.jsÿc0] Begin gear sharing");
		...
		if (HordeSettings.Debug.Verbose.sharing) print("[ÿc5Sharing.jsÿc0] Received all profiles gear offers");
		
		var sharingProfile = this.getSharingProfile();
		if (sharingProfile) {
			do {
				if (sharingProfile === me.profile) {
					if (HordeSettings.Debug.Verbose.sharing) print("[ÿc5Sharing.jsÿc0] Offering " + this.sharableGear.length + " items");
					...
					for (var i = 0 ; i < this.sharableGear.length ; i += 1) {
						...
						var targetProfiles = checkResult.result === 1 && (!!checkResult.tier || !!checkResult.merctier)
						...
						if (winner) {
							if (itemToShare.location === 7 ) {
								if (Storage.Inventory.CanFit(itemToShare)) Storage.Inventory.MoveTo(itemToShare);
							}
							
							this.giveItem(winner, itemToShare);
						} else {
							if (HordeSettings.Debug.Verbose.sharing) {
								print("[ÿc5Sharing.jsÿc0] Nobody needs " + Pickit.itemColor(itemToShare).trim + itemToShare.name.trim());
							}
						}
					...
					if (this.sharableGear.length == 0) {
						if (HordeSettings.Debug.Verbose.sharing) print("[ÿc5Sharing.jsÿc0] Finished offering gear");
						sharingProfile = this.getSharingProfile();
					} else if (HordeSettings.Debug.Verbose.sharing) {
						print("[ÿc5Sharing.jsÿc0] Still have " + this.sharableGear.length + " items to offer");
					}
				} else {
					if (HordeSettings.Debug.Verbose.sharing) {
						print("[ÿc5Sharing.jsÿc0] Waiting for " + sharingProfile + " to offer items");
					}
			...
			if (HordeSettings.Debug.Verbose.sharing) print("[ÿc5Sharing.jsÿc0] All offers processed");
		} else {
			if (HordeSettings.Debug.Verbose.sharing) print("[ÿc5Sharing.jsÿc0] No gear to share");
		}

		if (HordeSettings.Debug.Verbose.sharing) print("[ÿc5Sharing.jsÿc0] End gear sharing");
					...
					if (!this.isInOfferedGearHistory(item)) {
						...
						if (pickResult.result === 0 || pickResult.result === 1) {
							//if (!AutoMule.isIngredient(item)) {
								if (HordeSettings.Debug.Verbose.sharing) {
									print("[ÿc5Sharing.jsÿc0] Can share " + Pickit.itemColor(item).trim() + item.name.trim() + " (gid: " + item.gid + " - result: " + pickResult.result + " - line: " + pickResult.line);
								}
		...
		if (!!item) {
			do {
				if ([3,7].indexOf(item.location) !== -1 && (!AutoMule.isIngredient(item) || !this.hasGearPriority(requestProfile))) {
					...
					if (runeListIndex !== -1) offerList.push(item.classid);
				}
			} while(item.getNext());
		}
			...
			if (profile != me.profile) {
				...
				if (this.runeOffers[profile].length > 0) {
					for (var j = 0 ; j < this.runeOffers[profile].length ; j += 1) {
						...
						else if (this.isInCubingNeedlist(this.runeOffers[profile][j])) requestedRunes.push(this.runeOffers[profile][j]);
						//Accept keys
				...
				if (requestedRunes.length > 0) this.requestDropRune(profile, requestedRunes);
			}
		}
	...
	requestDropRune: function(profile, runeList) {
		...
		if (HordeSettings.Debug.Verbose.sharing) print("[ÿc5Sharing.jsÿc0] Picking runes from " + profile);
	...
	dropRune: function(profile, runeList) {
		...
		if (HordeSettings.Debug.Verbose.sharing) {
			print("[ÿc5Sharing.jsÿc0] Waiting for " + profile + " to pick " + itemsToDrop.length + " runes");
		}
		...
		if (HordeSettings.Debug.Verbose.sharing) print("[ÿc5Sharing.jsÿc0] Runes picked by " + profile);
	...
	getCraftingNeedList: function() {
		...
		for (var i = 0 ; i < Cubing.neededIngredients.length ; i += 1) {
			...
			if ((ingredientClassId >= 610 && ingredientClassId <= 642) //runes
				...
				if (needList.indexOf(ingredientClassId) === -1) needList.push(ingredientClassId);
			}
		}
	...
	shareRunes: function() {
		if (HordeSystem.teamSize === 1 || !this.isRuneSharingEnabled()) return;
		...
		if (HordeSettings.Debug.Verbose.sharing) {
			print("[ÿc5Sharing.jsÿc0] Begin rune sharing");
		}
		...
		if (HordeSettings.Debug.Verbose.sharing) print("[ÿc5Sharing.jsÿc0] Received all rune need lists");
		
		var requestProfile = this.getRuneRequestProfile();
		if (requestProfile) {
			do {
				if (requestProfile === me.profile) {
					if (HordeSettings.Debug.Verbose.sharing) print("[ÿc5Sharing.jsÿc0] Waiting offer list");
					while(!this.hasReceivedAllProfilesOfferList()) {
						delay(50);
						Party.wholeTeamInGame();
					}
					
					if (HordeSettings.Debug.Verbose.sharing) print("[ÿc5Sharing.jsÿc0] Processing offer list");
					
					this.processOfferList();
				} else {
					if (HordeSettings.Debug.Verbose.sharing) print("[ÿc5Sharing.jsÿc0] Sending offers to " + requestProfile);
					...
					if (HordeSettings.Debug.Verbose.sharing) {
						print("[ÿc5Sharing.jsÿc0] Waiting " + requestProfile + " to process need list");
					}
		...
		if (HordeSettings.Debug.Verbose.sharing) print("[ÿc5Sharing.jsÿc0] Waiting for all profiles to finish rune sharing");
		
		Party.waitSynchro("end_rune");
		
		if (HordeSettings.Debug.Verbose.sharing) print("[ÿc5Sharing.jsÿc0] End rune sharing");
