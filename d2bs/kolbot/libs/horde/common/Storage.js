...
var HordeStorage = {
	organize: function () {
		let stash = [{ item: me.getItem(549), x: 0, y: 0 }]; // Cube
		let inventory = [{ item: Role.getTpTome(), x: 0, y: 0 }]; // TP Tome

		// Keys if any
		let keys = me.findItems(543, 0, 3);
	...
	// Remove items that can no longer be worn due to a change in Dexterity/Strength.
	// Will try to place in Inventory to be checked against Pickit.
	// Clearinventory sells to NPC if there is no Pickit match.
	removeUnwearableItems: function () {
		let item = me.findItem(null, 1, 1); // Equipped item.
					...
					if (Storage.Inventory.CanFit(item)) {
						while (!Storage.Inventory.MoveTo(item)) { // Try to move the item more than once.
							...
							while (me.itemoncursor) {
								...
								Packet.dropItem(item); // Why not sell? Where's Packet.dropItem?
								item.drop(); // From cubing.js
								delay(me.ping * 2 + 100);
							}
						}
	...
	stashQuestItems: function () {
		// Check to make sure this includes all quest items that need to be moved to Stash.
		// Doesn't actually need the Horadric Cube to be in this list?
		let questStuff = [91, 92, 173, 174, 521, 524, 525, 553, 554, 555];

		for (let i = 0; i < questStuff.length; i += 1) {
			let item = me.getItem(questStuff[i]);
