...
var HordeSystem = {
	...
	setupBuild: function (buildName) {
		...
		let statBuild = StatsBuilds[this.build.statsBuild];
		let skillsBuild = SkillsBuilds[this.build.skillsBuild];
	...
	trimBaseName: function (name) {
		let splitted = name.split(' '), result = "";
	...
	setupRunewordLocation: function (locationName, runewordLocation, merc) {
		let runewords = Object.keys(runewordLocation), runeword, pickitLine;
		let bodyLocMap = { "auricshields": 5, "shield": 5, "armor": 3, "helm": 1 };
		let defaultBodyLoc = 4;

		for (let i = 0; i < runewords.length; i += 1) {
			runeword = runewordLocation[runewords[i]];
			if (runeword.skipIf === undefined || runeword.skipIf === "" || !eval(runeword.skipIf)) {
				let runewordBodyLoc = defaultBodyLoc;

				if (!!bodyLocMap[locationName]) {
					runewordBodyLoc = bodyLocMap[locationName];
				}

				let currentItemTier = merc ? Item.getEquippedItemMerc(runewordBodyLoc).tier : Item.getEquippedItem(runewordBodyLoc).tier;
				...
				if (!runeword.tier || currentItemTier < runeword.tier) {
					...
					let itemTypeCondition = runeword.typeCondition;
					...
					for (let j = 0; j < runeword.bases.length; j += 1) {
						let lowerCaseName = this.trimBaseName(runeword.bases[j]);
	...
	setupRunewordCategory: function (category, merc) {
		let locations = Object.keys(category);
	...
	setupRunewords: function (runewordProfile) {
		...
		if (RunewordProfile.runes.stock) {
			...
			for (let i = 0; i < this.pushedRunewords.length; i += 1) {
				let runeword = this.pushedRunewords[i];
				
				for (let j = 0; j < runeword.length; j += 1) {
			...
			let runes = Object.keys(requirements);

			for (let i = 0; i < runes.length; i += 1) {
				let rune = requirements[runes[i]];
				let runeLine = "[name] == " + rune.rune + " ## [maxquantity] == " + (RunewordProfile.runes.stockAllRecipes ? rune.quantity : 1);
	...
	setupPickits: function () {
		if (!!this.team.commonPickits) {
			print("[ÿc8Horde.jsÿc0] Setup common pickits");

			for (let i = 0; i < this.team.commonPickits.length; i += 1) {
				let conditionResult = true;
				let commonPickit = this.team.commonPickits[i];

				if (!!commonPickit.condition && commonPickit.condition !== "") {
					conditionResult = eval(commonPickit.condition);
					print("[ÿc8Horde.jsÿc0] Evaluate " + commonPickit.pickit + ", result: " + conditionResult);
				}

				if (conditionResult) {
					print("[ÿc8Horde.jsÿc0] Add common pickit: " + commonPickit.pickit);
	...
	setupConfig: function (teamName, oog) {
		print("[ÿc8Horde.jsÿc0] Setup config " + me.profile + "[" + teamName + "]");
	...
	getGameDifficulty: function () {
		...
		if (selectedDifficulty === undefined) {
			HordeDebug.logUserError("[ÿc8Horde.jsÿc0] Wrong difficulty settings in your team. Compare with template file. Picking profile one ...");
	...
	toggleThreadsPause: function () {
		let scripts = ["tools/townchicken.js", "tools/antihostile.js", "tools/party.js", "tools/rushthread.js", "tools/toolsthread.js"];
	...
	onToolThreadQuit: function () {
		...
		if (!this.team.instantQuitList) {
			...
			let minDelay = baseDelay + (baseDelay * (this.getTeamIndex(me.profile) + 1));
