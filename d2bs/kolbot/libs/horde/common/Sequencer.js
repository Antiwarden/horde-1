...
var Sequencer = {
	...
	getSequenceParams: function(sequence, timeline) {
		if (timeline == this.before) return this.beforeSequences[me.diff][sequence];
		else if (timeline == this.quest) return this.questSequences[me.diff][sequence];
		else if (timeline == this.after) return this.afterSequences[me.diff][sequence];
	...
	runSequence: function(sequence, timeline) {		
		...
		if (!isIncluded(sequenceInclude)) {
			if (!include(sequenceInclude)) throw new Error("Could not find sequence " + sequence);
		}
		...
		if (sequenceResult === this.ok) {			
			print("[ÿc:Debugÿc0] Next sequence: " + this.getTimelineName(timeline) + " " + sequence);
