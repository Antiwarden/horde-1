...
function baal(mfRun) {
	...
	this.monsterChickenCheck = function(){
		let quitOnDolls = me.diff === 2 && me.playertype;
		let quitOnSouls = me.diff === 2 && me.playerType;
	...
	this.clearThrone = function () {
		let monList = [];
		let pos = [15094, 5022, 15094, 5041, 15094, 5060, 15094, 5041, 15094, 5022];
	...
	for (let questTry = 0; questTry < 10; questTry += 1) {
		if (me.getQuest(40, 0)) {
			quest = true;
	...
	if (Role.teleportingChar) {
		try {
			...
			for (let baalfail = 0; baalfail < 10; baalfail =+1) {
				...
				if (me.area === 131) {
					Pather.moveToExit([130, 129], true, false); // Needs work!
					Waypoint.clickWP();
	...
	if (Role.teleportingChar && Party.hasReachedLevel(28)) {
		Pather.moveToExit(130, true, false);
		//Pather.moveTo(15121, 5237);

    Pather.moveToExit(131, true, false);
	...
	let ogFindItem = Config.FindItem;
	...
	let hasQuestDropProfile = me.diff === 2 && HordeSystem.questDropProfile !== "";
	...
	if (!Role.questDropChar || !hasQuestDropProfile) {
		...
		let baalded = false;
		let baalloop = getTickCount() + 3 * 6e4;
