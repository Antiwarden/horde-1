...
function main() {
	...
	this.keyEvent = function (key) {
		...
		case 107: // Numpad + = Log stats
			...
			break;
		case 101: // Numpad 5 = Force automule check
			...
			break;
		case 102: // Numpad 6 = Log character to char viewer
			MuleLogger.logChar();
			me.overhead("Logged char: " + me.name);

			break;
		case 109: // Numpad - = Log our items to item log (Should this try to get nearest player? Isn't that what it was meant for?)
			Misc.spy(me.name);

			break;
		case 110: // Numpad decimal point = Dump item info
			say("/fps");
	...
	this.gameEvent = function (mode, param1, param2, name1, name2) {
		...
		case 0x03: // "%Name1(%Name2) left our world. Diablo's minions weaken."
			if ((typeof Config.QuitList === "string" && Config.QuitList.toLowerCase() === "any") ||
				...
				// we need to stall in town, not on the battlefield
				this.togglePause();
		...
		case 0x11: // "%Param1 Stones of Jordan Sold to Merchants"
			...
			if (Config.SoJWaitTime && me.gametype === 1) { // 1 = expansion
				D2Bot.printToConsole(param1 + " Stones of Jordan Sold to Merchants on IP " + me.gameserverip.split(".")[3], 7); // 7 = sdk.colors.D2Bot.DarkGold
		...
		case 0x12: // "Diablo Walks the Earth"
			...
			if (Config.StopOnDClone && me.gametype === 1) { // 1 = expansion
	...
	while (true) {
				...
				if (Config.IronGolemChicken > 0 && me.classid === 2) {
					...
					if (ironGolem) {
						if (ironGolem.hp <= Math.floor(128 * Config.IronGolemChicken / 100)) { // ironGolem.hpmax is bugged with BO
							D2Bot.printToConsole("Iron Golem Chicken in " + Pather.getAreaName(me.area), 9);
							D2Bot.updateChickens();
							this.exit();
		...
		if (quitFlag && canQuit && (typeof quitListDelayTime === "undefined" || getTickCount() >= quitListDelayTime)) {
			print("ÿc8Run duration: ÿc2" + Math.floor((getTickCount() - me.gamestarttime) / 1000 / 60) + " minutes");
			...
			try {
				...
				print("[ÿc9Warningÿc0] Error in HordeSystem.onToolThreadQuit handler");
