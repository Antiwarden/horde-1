...
function main () {
	...
	this.skillInValidRange = function(id) {
		switch (me.classid) {
		case 0:
			return	6 <= id && id <= 35; // Amazon
		case 1:
			return 36 <= id && id <= 65; // Sorceress
		case 2:
			return 66 <= id && id <= 95; // Necromancer
		case 3:
			return 96 <= id && id <= 125; // Paladin
		case 4:
			return 126 <= id && id <= 155; // Barbarian
		case 5:
			return 221 <= id && id <= 250; // Druid
		case 6:
			return 251 <= id && id <= 280; // Assassin
		default:
		}
	...
	this.gainedLevels = function() {
		return me.charlvl - prevLevel;
	};
	...
	this.canSpendPoints = function() {
		...
		if (debug) {
			AutoBuild.print("Stat points: " + unusedStatPoints + ", Skill points: " + unusedSkillPoints);
		}
	...
	this.spendStatPoints = function() {
		...
		if (len > unusedStatPoints) {
			...
			AutoBuild.print("ÿc9Warningÿc0: Number of stats specified in your build template at level " + me.charlvl + " exceeds the available unused stat points." +
			...
			if (id === -1) { continue; }
				...
				if (SPEND_POINTS) {
					if (!pointSpent) {
						...
						AutoBuild.print("ÿc9Warningÿc0: Attempt to spend point " + (i + 1) + " in " + STAT_ID_TO_NAME[id] + " may have failed!");
					} else if (debug) {
	...
	this.spendSkillPoints = function() {
		...
		if (len > unusedSkillPoints) {
			...
			AutoBuild.print("ÿc9Warningÿc0: Number of skills specified in your build template at level " + me.charlvl + " exceeds the available unused skill points." +
		...
		for (var i = 0; i < len; i++) {
			...
			if (requiredSkills.length > 0) {
				throw new Error("You need pre-requisite skills " + requiredSkills.join(", ") + " before adding " + skillName+errInvalidSkill + ".");
			}
	...
	try {
		...
		while (true) {
			...
			if (levels > 0 && (canSpendPoints() || Config.AutoSkill.Enabled || Config.AutoStat.Enabled)) {
				...
				print("[ÿc9AutoBuildThreadÿc0] Level up detected (" + prevLevel + " --> " + me.charlvl + ")");
